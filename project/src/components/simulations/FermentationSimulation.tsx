...
const FermentationSimulation: React.FC = () => {
  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (!containerRef.current) return;

    // Créer un iframe avec le code HTML complet modifié
    ...
    // Le code HTML complet de la simulation avec modification pour la diminution du volume
    const htmlContent = `
<!DOCTYPE html>
<html lang="fr">
<head>
...
<body>
    <div id="lab-container">
        ...
        <div class="test-tube water-tube" role="button" aria-label="Tube à essai - eau" onclick="selectTube('water')">
            <div class="tube-content" id="water-content" style="height: 100px; background: linear-gradient(to bottom, #4fc3f7, #29b6f6)"></div>
        ...
        <div class="test-tube glucose-tube" role="button" aria-label="Tube à essai - glucose" onclick="selectTube('glucose')">
            <div class="tube-content" id="glucose-content" style="height: 80px; background: linear-gradient(to bottom, #ffb74d, #ff9800)"></div>
        ...
        <div class="test-tube yeast-tube" role="button" aria-label="Tube à essai - levure" onclick="selectTube('yeast')">
            <div class="tube-content" id="yeast-content" style="height: 60px; background: linear-gradient(to bottom, #8bc34a, #689f38)"></div>
    ...
    <script>
        ...
        function selectTube(tubeType) {
            ...
            // Vérifier si le tube a encore du contenu
            if (experiment.tubeVolumes[tubeType] <= 0) {
                addObservation("⚠️ Ce tube est vide !");
                return;
            }
            ...
            // Diminuer le volume du tube
            const volumeToRemove = 20; // Volume prélevé
            experiment.tubeVolumes[tubeType] = Math.max(0, experiment.tubeVolumes[tubeType] - volumeToRemove);
            
            // Mettre à jour l'affichage du tube
            updateTubeDisplay(tubeType);
            
            addObservation(\`💉 \${getIngredientName(tubeType)} prélevé avec la pipette (Volume restant: \${experiment.tubeVolumes[tubeType]}%)\`);
        }

        /* ------------------------------- MISE À JOUR AFFICHAGE TUBES ----------------- */
        function updateTubeDisplay(tubeType) {
            const tubeContent = document.getElementById(tubeType + "-content");
            const volume = experiment.tubeVolumes[tubeType];
            
            if (volume <= 0) {
                tubeContent.style.height = "0px";
                tubeContent.style.background = "transparent";
                addObservation(\`⚠️ Le tube \${getIngredientName(tubeType)} est maintenant vide\`);
            } else {
                tubeContent.style.height = volume + "px";
                // Ajuster la couleur selon le volume restant
                const alpha = Math.max(0.3, volume / 100);
                const baseColor = colors[tubeType];
                tubeContent.style.background = \`linear-gradient(to bottom, \${baseColor}\${Math.floor(alpha * 255).toString(16).padStart(2, '0')}, \${baseColor}\${Math.floor(alpha * 0.8 * 255).toString(16).padStart(2, '0')})\`;
            }
        }
        ...
        function resetExperiment() {
            ...
            // Réinitialiser l'affichage des tubes
            updateTubeDisplay('water');
            updateTubeDisplay('glucose');
            updateTubeDisplay('yeast');
...
export default FermentationSimulation; 